...
echo "Building Sensimul web image from ${SENSIMUL_DIR}"
docker build --platform "${PLATFORM}" --build-arg APP_NAME=web -t "${SENSIMUL_ECR_REPOSITORY_URL}:${IMAGE_TAG}" "${SENSIMUL_DIR}"

echo "Pushing ${SENSIMUL_ECR_REPOSITORY_URL}:${IMAGE_TAG}"
docker push "${SENSIMUL_ECR_REPOSITORY_URL}:${IMAGE_TAG}"

echo
echo "Pushed Sensimul web image:"
